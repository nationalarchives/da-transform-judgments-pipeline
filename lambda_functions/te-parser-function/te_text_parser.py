...
# move to env var
API_ENDPOINT = "https://7gwnzr88s0.execute-api.eu-west-2.amazonaws.com/default/dev-te-text-parser"
API_KEY = ""
...
def handler(event, context):
    ...
    * verify the number of extracted files matches the numbers in the 2 manufest files
    ...
    try:
        ...
        response = requests.request("POST", API_ENDPOINT, headers=headers, data=json.dumps(data))
        ...
        # xml
        with open(f'tmp/{event["output-message"]["consignment-reference"]}-te-xml.xml', 'w') as xml_file:
            ...
            S3.upload_file(f'tmp/{event["output-message"]["consignment-reference"]}-te-xml.xml', KEY_S3_PARSER_BUCKET, object_key)
        ...
        with open(f'tmp/{event["output-message"]["consignment-reference"]}-te-meta.json', 'w') as json_file:
            ...
            S3.upload_file(f'tmp/{event["output-message"]["consignment-reference"]}-te-meta.json', KEY_S3_PARSER_BUCKET, object_key)
        ...
        dest.copy(source, f"parsed/judgement/{event['output-message']['consignment-reference']}/{event['output-message']['number-of-retries']}/judgment.docx")
        ...
        dest = S3_resource.Bucket(KEY_S3_PARSER_BUCKET)
        dest.copy(source, f"parsed/judgement/{event['output-message']['consignment-reference']}/{event['output-message']['number-of-retries']}/bagit-info.txt")

        
        # place xml, meta json, bag it info in output message
        output["consignment-reference"] = event["output-message"]["consignment-reference"]
        output["s3-object-root"] = f"parsed/judgement/{event['output-message']['consignment-reference']}/{event['output-message']['number-of-retries']}"
        output["s3-parser-bucket"] = KEY_S3_PARSER_BUCKET
        output[KEY_PARSED_FILES]["xml"] = f"parsed/judgement/{event['output-message']['consignment-reference']}/{event['output-message']['number-of-retries']}/te-xml.xml"
        output[KEY_PARSED_FILES]["judgement"] = f"parsed/judgement/{event['output-message']['consignment-reference']}/{event['output-message']['number-of-retries']}/judgement.docx"
        output[KEY_PARSED_FILES]["meta"] = f"parsed/judgement/{event['output-message']['consignment-reference']}/{event['output-message']['number-of-retries']}/te-meta.json"
        output[KEY_PARSED_FILES]["bagit-info"] = f"parsed/judgement/{event['output-message']['consignment-reference']}/{event['output-message']['number-of-retries']}/bagit-info.txt"
